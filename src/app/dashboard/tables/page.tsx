...
export default function TablesPage() {
  ...
  const handleTableUpdated = useCallback(
    (t: Partial<ApiDiningTable> & { id: string }) => {
      setTables((prev) =>
    ...
    <>
        ...
        <div className="mb-[18px] flex flex-wrap items-center justify-between gap-[10px]">
          <div>
            ...
            <div className="mt-0.5 text-xs text-text3">
              {tables.length} tables · {occupied} occupied · {free} free
            </div>
          </div>
          <div className="flex gap-2">
            <button
              onClick={() =>
                showToast(
                  `🖨 Preparing QR codes for all ${tables.filter((t) => t.active).length} active tables…`,
                )
              }
            ...
            <button
              onClick={() => checkSubscription("Add Table", openAddTable)}
        ...
        <div className="mb-[18px] flex items-center gap-4 rounded-[10px] border border-border bg-surface px-[18px] py-[15px] shadow-[0_1px_3px_rgba(0,0,0,.07)]">
          ...
          <div className="flex-1">
            ...
            <div className="text-xs text-text2">
              Each table gets a unique QR. Customer scans → browses menu →
              places order online. Print, laminate, place on table.
            </div>
          </div>
          <button
            onClick={() => showToast("🖨 Preparing QR codes…")}
            className="rounded-[7px] border border-border2 bg-transparent px-[11px] py-[5px] text-xs font-semibold text-text transition-all hover:bg-surface2"
          >
            Preview all →
          </button>
        </div>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-[repeat(auto-fill,minmax(158px,1fr))]">
            {tables.map((t) => (
              ...
              >
                ...
                <div className="mt-0.5 font-mono text-[10px] font-semibold uppercase tracking-[0.08em] text-text3">
                ...
                <div className="mt-[11px] flex gap-[5px]">
                  <button
                    ...
                    className="flex-1 rounded-md bg-accent py-[5px] text-center text-[10px] font-semibold text-white hover:bg-accent2"
                  >
                    QR
                  </button>
                  <button
                    onClick={() =>
                      checkSubscription("Edit Table", () => openEditTable(t.id))
                    }
            ...
            <div
              onClick={() => checkSubscription("Add Table", openAddTable)}
              className="flex min-h-[148px] cursor-pointer flex-col items-center justify-center rounded-xl border-[1.5px] border-dashed border-border2 bg-transparent p-4 text-text3 transition-all hover:border-accent hover:bg-accent-bg hover:text-accent"
            >
      ...
      {qrTable && (
        <div
          ...
          onClick={(e) => e.target === e.currentTarget && setQrTable(null)}
        >
          <div className="mx-4 w-full max-w-[360px] overflow-hidden rounded-2xl bg-surface shadow-[0_20px_60px_rgba(0,0,0,.12)] animate-slideUp sm:mx-0">
            ...
            <div className="p-6 text-center">
              ...
              <div className="mb-1 font-serif text-xl font-extrabold">
                {qrTable.label}
              </div>
            ...
            <div className="flex gap-2 border-t border-border px-5 py-[14px]">
              <button
                onClick={() => setQrTable(null)}
                className="flex flex-1 items-center justify-center rounded-lg border-[1.5px] border-border2 bg-transparent py-[9px] text-[13px] font-semibold text-text hover:bg-surface2"
              >
                Cancel
              </button>
              <button
              ...
              <button
                onClick={() => {
                  showToast(`📥 QR for ${qrTable.label} downloading…`);
                  setQrTable(null);
                }}
                className="flex flex-1 items-center justify-center rounded-lg bg-accent py-[9px] text-[13px] font-semibold text-white hover:bg-accent2"
              >
                ⬇ Download
              </button>
            </div>
          ...
          <div className="mx-4 w-full max-w-[400px] overflow-hidden rounded-[14px] bg-surface shadow-[0_20px_60px_rgba(0,0,0,.12)] animate-slideUp sm:mx-0">
            <div className="flex items-center justify-between border-b border-border px-5 py-4">
              <div className="text-sm font-bold">
                {editingId ? `Edit ${tblLabel}` : "Add New Table"}
              </div>
              <button
            ...
            <div className="p-5">
              <div className="mb-4">
                ...
                <input
                  ...
                  placeholder="e.g. Table 9 or VIP Booth"
                  value={tblLabel}
                  onChange={(e) => setTblLabel(e.target.value)}
                />
                <div className="mt-1 text-[11px] text-text3">
                  This label appears in owner notifications and order history
                </div>
              </div>
              <div className="mb-4">
                <label className="mb-[5px] block text-xs font-semibold text-text2">
                ...
                <div className="flex flex-wrap gap-2">
                  {capacityOptions.map((n) => (
                    <div
                      key={n}
                      onClick={() => setTblCap(n)}
                      className={`cursor-pointer rounded-[7px] border-[1.5px] px-[14px] py-[7px] text-[13px] font-semibold transition-all ${tblCap === n ? "border-accent bg-accent-bg text-accent" : "border-border hover:bg-surface2"}`}
                    >
                      {n}
                    </div>
                  ))}
                ...
                <div className="flex items-center justify-between rounded-lg border border-border bg-surface2 px-[13px] py-[10px]">
                  <div>
                    ...
                    <div className="mt-[1px] text-[11px] text-text3">
                      Inactive tables are hidden from QR list and bot
                    </div>
                  </div>
                  <button
                    onClick={() => setTblActive(!tblActive)}
                    className={`relative h-[18px] w-8 shrink-0 cursor-pointer rounded-[9px] border-none transition-colors ${tblActive ? "bg-green-mid" : "bg-border2"}`}
                  >
                    <div
                      className={`absolute top-[3px] h-3 w-3 rounded-full bg-white shadow-[0_1px_3px_rgba(0,0,0,.2)] transition-[left] ${tblActive ? "left-[17px]" : "left-[3px]"}`}
                    />
                  </button>
                </div>
              </div>
